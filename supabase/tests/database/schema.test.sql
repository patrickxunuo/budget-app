...
select ok(exists(select 1 from pg_constraint c join pg_class r on r.oid=c.conrelid join pg_namespace n on n.oid=r.relnamespace where n.nspname='public' and r.relname='budgets' and c.contype='c' and c.conname='budgets_amount_cents_safe'), 'DB-003 budgets has a named safe positive-cent check');
select ok(exists(select 1 from pg_constraint c join pg_class r on r.oid=c.conrelid join pg_namespace n on n.oid=r.relnamespace where n.nspname='public' and r.relname='budgets' and c.contype='c' and c.conname='budgets_month_range_valid'), 'DB-003 budgets has a named effective-month range check');
...
insert into public.categories(id,workspace_id,created_by,name,scope,owner_profile_id)
values ('40000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','10000000-0000-0000-0000-000000000001','Schema budget category','family',null);
select throws_ok($$insert into public.budgets(workspace_id,created_by,category_id,amount_cents,currency_code,effective_month,end_month,scope,owner_profile_id) values ('20000000-0000-0000-0000-000000000001','10000000-0000-0000-0000-000000000001','40000000-0000-0000-0000-000000000001',0,'CAD','2026-08-01',null,'family',null)$$,'23514',null,'DB-003 rejects non-positive budget cents');
select throws_ok($$insert into public.budgets(workspace_id,created_by,category_id,amount_cents,currency_code,effective_month,end_month,scope,owner_profile_id) values ('20000000-0000-0000-0000-000000000001','10000000-0000-0000-0000-000000000001','40000000-0000-0000-0000-000000000001',10000,'CAD','2026-08-01','2026-07-01','family',null)$$,'23514',null,'DB-003 rejects invalid budget month ranges');
