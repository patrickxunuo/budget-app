...
insert into public.manual_entries(id,workspace_id,created_by,last_edited_by,scope,owner_profile_id,kind,amount,currency_code,entry_date,description,category_id) values
('f2000000-0000-0000-0000-000000000001','b0000000-0000-0000-0000-000000000001','a0000000-0000-0000-0000-000000000001','a0000000-0000-0000-0000-000000000001','family',null,'income',10,'CAD',current_date,'Family manual','c0000000-0000-0000-0000-000000000001'),
('f2000000-0000-0000-0000-000000000002','b0000000-0000-0000-0000-000000000001','a0000000-0000-0000-0000-000000000002','a0000000-0000-0000-0000-000000000002','personal','a0000000-0000-0000-0000-000000000002','income',20,'CAD',current_date,'Private manual','c0000000-0000-0000-0000-000000000002');
...
select lives_ok($$select public.create_manual_entry('family','income',3,current_date,'Temporary manual','c0000000-0000-0000-0000-000000000001',null)$$,'DB-009 active members create Family manual entries through the validating RPC');
select lives_ok($$select public.update_manual_entry('f2000000-0000-0000-0000-000000000001','income',10,current_date,'Updated family manual','c0000000-0000-0000-0000-000000000001',null)$$,'DB-009 active members update Family manual entries through the validating RPC');
select lives_ok($$select public.soft_delete_manual_entry((select id from public.manual_entries where description='Temporary manual'),true)$$,'DB-009 active members soft-delete Family manual entries through the validating RPC');
...
select throws_ok($$select public.create_manual_entry('family','income',4,current_date,'Bad family category','c0000000-0000-0000-0000-000000000002',null)$$,'23503',null,'DB-009 Family manual entries cannot reference a Personal category');
...
select lives_ok($$select public.create_manual_entry('personal','income',7,current_date,'Temporary private manual',(select id from public.categories where name='Temporary Personal'),null)$$,'DB-009 owners create Personal manual entries');
...
select is((public.update_manual_entry('f2000000-0000-0000-0000-000000000002','income',999,current_date,'Owner takeover','c0000000-0000-0000-0000-000000000002',null)).id,null,'DB-009 family owner cannot update another member Personal manual entry');
