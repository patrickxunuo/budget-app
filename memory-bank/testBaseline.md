...
## GH-66 Mobile Transactions Information-First

- [x] Service, route, component, shell-composition, shared-select, and deterministic-seed acceptance covers source-authorized Plaid/Manual details, sanitized errors, sole-page-heading hierarchy, mobile compact totals, modal advanced filters/chips with portaled focus and nested Escape, breakpoint cleanup, wrapping date-grouped exceptional-badge rows, read-only detail loading/retry/focus behavior, responsive 10/50-row reveal, retained failures, desktop controls, and announcements. Final focused verification is 71/71 and the complete Vitest suite is 1009/1009; lint, typecheck, formatting, development startup, production build, and independent review are green.
- [x] `e2e/dashboard.spec.ts` adds no-mock desktop/mobile journeys for sole-heading hierarchy, overflow and touch targets, mobile filter-sheet operation/chips/portal focus, deterministic mixed-source grouping and exceptional badges, 10-row expansion, complete details/failure recovery, desktop retention, and screenshot checkpoints. The idempotent financial seed now supplies 23 All-lines rows across at least three dates without changing exact categorized spending expectations; Playwright statically discovers all 26 dashboard cases.
- The configured one-pass `npm run test:e2e` attempt reached no browser cases because Next development startup rejected the initial detail route as a dynamic-segment collision with the existing category route. The endpoint was moved to the static `/api/transactions/detail/:source/:id` namespace; focused tests, `next dev` startup, route type generation, and a production build are green afterward. Per the single-run Phase 5 contract the full browser suite was not run a second time, so no GH-66 browser pass or screenshot is claimed locally.
