...
- Live regions: mount `role="status"` regions empty and toggle only their contents — a region inserted with its text already present is not announced. The shell always carries two, so browser assertions on a page's own status must be scoped.
