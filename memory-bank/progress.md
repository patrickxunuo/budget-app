...
- 2026-08-14T10:10Z [PLANNED] GH-32 (route-level loading skeletons and navigation pending feedback) is being built on `feat/GH-32-route-loading-skeletons`, cut from `main` at `045dc7b`. Five features: shared skeleton primitives plus a `.skeleton` shimmer with its own `prefers-reduced-motion` rule; six segment `loading.tsx` files under `src/app/(app)/` that reproduce each route's `<main>` wrapper, container width, and real structure; a fixed-size `useLinkStatus` pending affordance in the rail and the bottom bar; a restyled root loader; and component plus browser coverage. `#31` is still open, so this lands first and `#31` owns updating the dashboard skeleton.
