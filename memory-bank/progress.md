...
Every issue through GH-14 is closed — GH-14 landed in `7811e75`, and `main` is at `045dc7b` after a run of Dependabot bumps. The application is deployed to production at `piggy-budget-app.vercel.app` against a hosted Supabase project and Plaid Sandbox; a real Canadian institution links, activates, and syncs end to end, and the app installs to a home screen. GH-32 (route-level loading skeletons) is open as PR #48. GH-15 is the remaining open v1.0 issue; GH-26 (themed select and searchable dropdown polish), GH-31 (read-only month-to-date dashboard), and GH-33 (pending states on mutating controls) are open with no milestone.
