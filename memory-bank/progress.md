...
Every issue through GH-13 is closed. The application is deployed to production at `piggy-budget-app.vercel.app` against a hosted Supabase project and Plaid Sandbox; a real Canadian institution links, activates, and syncs end to end, and the app installs to a home screen. GH-14 is in flight on `feat/GH-14-testing-security-hardening`. GH-30 is in review in [PR #49](https://github.com/patrickxunuo/budget-app/pull/49) and blocks #31 (refactor the dashboard into a read-only month-to-date overview). GH-15 is the remaining open v1.0 issue; GH-26 (themed select and searchable dropdown polish) and #33 (shared pending/disabled primitive) are open with no milestone — both now have a concrete consumer in the `/transactions` exploration controls.
